...
for name in images:
    ...
    # 1. Sample paper background intensity dynamically
    # ROI mask with 25mm margin
    ...
    # 2. Get object body median intensity
    ...
    # Threshold is 25% of the way from the object body median to the paper median
    ...
    for c in contours:
        ...
        if min_hole_area <= area <= max_hole_area:
            ...
            if circularity >= 0.45 and mean_val >= 0.45 * paper_median:
                print(f"  Detected Hole: area={area/100.0:.2f} mm^2, circularity={circularity:.2f}, mean={mean_val:.1f}")
    ...
    # Save
    out_mask_path = os.path.join(scratch_dir, f"{name}_no_inside_mask.png")
    ...
    cv2.imwrite(os.path.join(scratch_dir, f"{name}_no_inside_overlay.png"), overlay)
