...
for name in images:
    ...
    # Rebuild a completely solid mask by taking only the outer boundary of the SAM mask
    solid_mask = np.zeros_like(warped_mask)
    outer_contours, _ = cv2.findContours(warped_mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    if len(outer_contours) > 0:
        ...
        solid_mask = warped_mask.copy() # fallback
    # -------------------------------
    ...
    pts_object = np.argwhere(solid_mask > 0)
    ...
    body_pixels = warped_gray[(solid_mask > 0) & (warped_gray < 0.9 * paper_median)]
    ...
    detected_holes = cv2.bitwise_and(thresh_img, thresh_img, mask=solid_mask)
        ...
        if 200 <= area <= 500000:
            ...
            if (is_large_hole or is_valid_small_hole) and (mean_val >= 0.45 * paper_median):
                ...
                if ratio >= 0.70:
                    ...
                    cv2.circle(solid_mask, (cx, cy), r_robust, 0, -1)
                else:
                    print(f"  Detected RAW contour hole: area={area/100.0:.2f} mm^2, ratio={ratio:.2f}")
                    cv2.drawContours(solid_mask, [c], -1, 0, -1)
                    
    # Save final
    cv2.imwrite(os.path.join(scratch_dir, f"solid_punched_{name}_mask.png"), solid_mask)
    overlay = warped_img.copy()
    overlay[solid_mask > 0] = [0, 255, 0]
    cv2.addWeighted(overlay, 0.4, warped_img, 0.6, 0, overlay)
    cv2.imwrite(os.path.join(scratch_dir, f"solid_punched_{name}_overlay.png"), overlay)
