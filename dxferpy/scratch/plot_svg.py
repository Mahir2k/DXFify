...
def vectorize_contour_debug(contour, height):
    ...
    approx_idx = []
    for pt in pts:
        dists = (contour[:, 0, 0].astype(float) - pt[0])**2 + (contour[:, 0, 1].astype(float) - pt[1])**2
    ...
    for j in range(num_v):
        hist[int(round(seg_angles[j])) % num_bins] += seg_lengths[j]
    ...
    for j in range(num_v):
        ...
        for di, d in enumerate(dom_dirs):
            diff = min(abs(seg_angles[j] - d), 180 - abs(seg_angles[j] - d))
    ...
    while j < num_v:
        ...
        run_end = j
        if seg_dir != -1:
            while run_end < num_v - 1 and snapped_dir_idx[(run_end + 1) % num_v] == seg_dir: run_end += 1
    ...
    for j in range(num_m):
        ...
        if snap1 != -1 and snap2 != -1:
            ...
            if abs(det) < 1e-6:
                ...
                if np.hypot(ix - merged_pts[j][0], iy - merged_pts[j][1]) < 50.0: refined.append((ix, iy))
                else: refined.append(merged_pts[j])
...
for contour in contours:
    if cv2.contourArea(contour) > 10000:
        pts, refined = vectorize_contour_debug(contour, mask.shape[0])
        svg = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1000 2000">'
        
        # DP points in green
        svg += '<polygon points="' + ' '.join(f"{p[0]},{p[1]}" for p in pts) + '" fill="none" stroke="green" stroke-width="5" />'
        # Refined points in red
        svg += '<polygon points="' + ' '.join(f"{p[0]},{p[1]}" for p in refined) + '" fill="none" stroke="red" stroke-width="2" />'
        
        svg += '</svg>'
        with open("/home/aledawizard/.gemini/antigravity/brain/13e2c01f-bc24-44bb-9f2a-47de4ccf7434/debug.svg", "w") as f:
            f.write(svg)
        break
