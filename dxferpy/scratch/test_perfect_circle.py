...
# Load the hybrid masks we created in the previous step
for name in images:
    mask_path = os.path.join(scratch_dir, f"hybrid_{name}.jpg") # wait, the previous script saved them as hybrid_{img_name}
    img = cv2.imread(f"samples/{name}.jpg")
    
    # Let's detect the markers to get homography and warped image first
    ...
    # We load the mask BEFORE hole punching (rectified_mask_raw)
    # Let's rebuild the SAM mask and warp it
    ...
    paper_median = np.median(warped_gray[paper_sample_mask > 0])
    body_median = np.median(warped_gray[(warped_mask > 0) & (warped_gray < 0.9 * paper_median)])
    ...
    for c in hole_contours:
        ...
        if 200 <= area <= 500000:
            ...
            if (is_large_hole or is_valid_small_hole) and (mean_val >= 0.45 * paper_median):
                if circularity >= 0.45:
                    # Circular hole! Grab centroid and estimate robust radius
                    M = cv2.moments(c)
                    cx = int(M["m10"] / M["m00"])
                    cy = int(M["m01"] / M["m00"])
                    
                    # Compute distances from centroid to all contour points
                    pts_c = c.reshape(-1, 2)
                    dists = np.sqrt((pts_c[:, 0] - cx) ** 2 + (pts_c[:, 1] - cy) ** 2)
                    
                    # Use 90th percentile to get non-wobbly radius (ignoring the wobbly shadow sides which are shorter)
                    r_robust = int(np.percentile(dists, 92))
                    print(f"  Punched CIRCULAR hole: area={area/100.0:.2f} mm^2, circ={circularity:.2f}, r={r_robust/10.0:.2f} mm")
                    cv2.circle(punched_mask, (cx, cy), r_robust, 0, -1)
                else:
                    # Non-circular (pill shape) -> Keep raw contour!
                    print(f"  Punched RAW contour hole: area={area/100.0:.2f} mm^2, circ={circularity:.2f}")
                    cv2.drawContours(punched_mask, [c], -1, 0, -1)
                    
    # Save
    cv2.imwrite(os.path.join(scratch_dir, f"circle_punched_{name}_mask.png"), punched_mask)
    overlay = warped_img.copy()
    overlay[punched_mask > 0] = [0, 255, 0]
    cv2.addWeighted(overlay, 0.4, warped_img, 0.6, 0, overlay)
    cv2.imwrite(os.path.join(scratch_dir, f"circle_punched_{name}_overlay.png"), overlay)
