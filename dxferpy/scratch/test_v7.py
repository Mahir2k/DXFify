...
def vectorize_contour_v7(contour, height):
    n = len(contour)
    ...
    if not dom_dirs: return [(p[0], float(height - p[1])) for p in pts]
    
    # Snap threshold: 10 degrees (preserves curves, snaps straight edges)
    snapped_dir_idx = []
    for j in range(num_v):
    ...
    # Merge consecutive SAME direction snapped segments
    ...
    if num_m < 3: return [(p[0], float(height - p[1])) for p in pts]
    ...
    for j in range(num_m):
        ...
        if snap1 != -1 and snap2 != -1:
            ...
            if abs(det) < 1e-6:
                ...
                refined.append((ix, iy) if np.hypot(ix - merged_pts[j][0], iy - merged_pts[j][1]) < 50.0 else merged_pts[j])
...
for name in ['wood_broken', 'phone_case']:
    img_path = f"output/rgba_{name}.png"
    ...
    for i, contour in enumerate(contours):
        ...
        if hier[0][i][3] != -1 and area < 500: continue
        if hier[0][i][3] == -1 and area < 100: continue
        pts = vectorize_contour_v7(contour, mask.shape[0])
        if pts: msp.add_lwpolyline(pts, close=True)
    doc.saveas(f"scratch/{name}_v7.dxf")
print("Done")
