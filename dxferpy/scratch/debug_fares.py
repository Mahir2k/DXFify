...
img = cv2.imread('output/rgba_fares.png', cv2.IMREAD_UNCHANGED)
mask = img[:, :, 3]
...
print(f"Peak 1: {peak1}, height: {hist_smooth[peak1]}, total: {np.sum(seg_lengths)}, 5% = {0.05*np.sum(seg_lengths)}")
