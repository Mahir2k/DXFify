...
img = cv2.imread('output/rgba_airpod_case.png', cv2.IMREAD_UNCHANGED)
mask = img[:, :, 3]
...
print(f"Peak 1: {peak1}, height: {hist_smooth[peak1]}, total: {np.sum(seg_lengths)}, 1% = {0.01*np.sum(seg_lengths)}")
